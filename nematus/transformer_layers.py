...
class EmbeddingLayer(object):
    ...
    def __init__(self, vocabulary_size, embedding_size, hidden_size, float_dtype, name, glove_vectors=None):
        # Set arguments
        ...
        if self.glove_vectors is not None:
            self.glove_embedding = tf.convert_to_tensor(np.load(self.glove_vectors))
            """
            with tf.variable_scope(self.name):
                init = tf.constant(np.load(self.glove_vectors))
                self.glove_embedding = tf.get_variable(name='glove_embedding',
		    					dtype=float_dtype,
							initializer=init,
							trainable=False)
            """
    ...
    def get_glove_embed(self, one_hot_inputs):
        if self.glove_vectors is not None: 
            embeddings = tf.nn.embedding_lookup(self.glove_embedding, one_hot_inputs)
            return embeddings
        return None
