...
class ConfigSpecification:
    ...
    def _define_param_specs(self):
        ...
        # Add command-line parameters for the 'pretrain' group.

        group = param_specs['pretrain']

        group.append(ParameterSpecification(
            name='pretrain', default=False,
            visible_arg_names=['--pretrain'],
            action='store_true'))

        group.append(ParameterSpecification(
            name='pretrain_vectors', default=None,
            visible_arg_names=['--pretrain_vectors'],
            type=str, metavar='PATH'))

        group.append(ParameterSpecification(
            name='pretrain_vocab', default=None,
            visible_arg_names=['--pretrain_vocab'],
            type=str, metavar='PATH'))

        group.append(ParameterSpecification(
            name='pretrain_tsne', default=None,
            visible_arg_names=['--pretrain_tsne'],
            type=str, metavar='PATH'))

        group.append(ParameterSpecification(
	    name='utf8_type', default='utf8',
	    visible_arg_names=['--utf8_type'],
	    type=str, choices=['utf8', 'md5']))

        group.append(ParameterSpecification(
            name='u_embedding_size', default=8,
            visible_arg_names=['--u_embedding_size'],
            type=int, metavar='INT',
            help='u embedding layer size (default: %(default)s)'))

        group.append(ParameterSpecification(
            name='pre_source_embedding_size', default=2048,
            visible_arg_names=['--pre_source_embedding_size'],
            type=int, metavar='INT',
            help='pre embedding size (default: %(default)s)'))

        group.append(ParameterSpecification(
            name='pre_source_ffn_middle_size', default=2048,
            visible_arg_names=['--pre_source_ffn_middle_size'],
            type=int, metavar='INT',
            help='pre source ffn middle size (default: %(default)s)'))

        group.append(ParameterSpecification(
            name='pre_source_embed_cross', default=False,
            visible_arg_names=['--pre_source_embed_cross'],
            action='store_true',
            help='pre_source_embed_cross (default: %(default)s)'))
