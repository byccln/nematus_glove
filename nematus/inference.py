...
class InferenceModelSet(object):
    ...
    def beam_search(self, session, px, x, x_mask, beam_size,
        ...
        if not cached_graph_is_usable():
            self._cached_beam_search_graph = \
                self._beam_search_graph_type(self._models, beam_size,
                                             normalization_alpha)
        return self._beam_search_func(session, self._models, px, x, x_mask,
                                      beam_size, normalization_alpha,
                                      self._cached_beam_search_graph)

    def decode(self, session, px, x, x_mask, beam_size,
               normalization_alpha=0.0):
        ...
        if self._models[0].sampling_utils.translation_strategy == 'beam_search':
            return self.beam_search(session, px, x, x_mask, beam_size, normalization_alpha)
...
def translate_file(input_file, output_file, session, models, configs,
    ...
    def translate_maxibatch(maxibatch, model_set, num_to_target,
        ...
        try:
            pre_minibatches, minibatches, idxs = util.read_all_lines(configs[0], maxibatch,
                                                    minibatch_size)
        ...
        for px, x in zip(pre_minibatches, minibatches):
            ...
            px, x, x_mask, _, _ = util.prepare_data(x, y_dummy, configs[0].factors,
                                                px, maxlen=None)
