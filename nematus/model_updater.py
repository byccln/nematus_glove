...
class ModelUpdater(object):
    ...
    def update(self, session, px, x, x_mask, y, y_mask, write_summary):
        ...
        split_px, split_x, split_x_mask, split_y, split_y_mask, weights = \
            self._split_and_pad_minibatch(px, x, x_mask, y, y_mask, start_points)
        ...
        for i in range(0, len(split_x), len(self._replicas)):
            ...
            for j in range(len(self._replicas)):
                ...
                feed_dict[self._replicas[j].inputs.px] = split_px[i+j]
    ...
    def _split_minibatch_into_n(self, x_mask, y_mask, n):
        ...
        assert len(source_lengths[0]) == len(target_lengths)
        num_sents = len(source_lengths[0])

        # Calculate the source + target batch sizes, then divide by n to get
        # the max size of each sub-batch.
        s_total = max(source_lengths[0]) * num_sents
    ...
    def _split_and_pad_minibatch(self, px, x, x_mask, y, y_mask, start_points):
        ...
        split_px = split_array(px, start_points)
        ...
        def trim_arrays_x(arrays, new_seq_lens):
            return [a[..., 0:l, :, :] for a, l in zip(arrays, new_seq_lens)]

        max_lens = [int(numpy.max(numpy.sum(m, axis=0))) for m in split_x_mask]
        split_px = trim_arrays(split_px, max_lens)
        split_x = trim_arrays_x(split_x, max_lens)
        split_x_mask = trim_arrays_x(split_x_mask, max_lens)
        ...
        return split_px, split_x, split_x_mask, split_y, split_y_mask, weights
